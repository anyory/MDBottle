        ...
        if (file_exists('./../p/index.php')) {
            ...
            foreach ($in as $key => $val) { /*先寻找是否有适配的PID*/
                if ($key === $page) {
                    ...
                    if ($_SESSION['log'] == 'yes') {
                        $edith = '<div><button onclick="window.location.href=\'./admin/edit.php?e=' . $page . '\'" class="mdui-btn mdui-shadow-2 mdui-ripple mdui-btn-icon"  target="_blank" mdui-tooltip="{content: \'编辑\'}"><i class="mdui-icon material-icons">edit</i></button>&emsp;<button onclick="window.location.href=\'./admin/edit.php?e=' . $page . '&t=del\'" target="_self" class="mdui-btn mdui-btn-icon mdui-shadow-2 mdui-ripple" mdui-tooltip="{content: \'删除\'}"><i class="mdui-icon material-icons">delete</i></button></div><br>';
